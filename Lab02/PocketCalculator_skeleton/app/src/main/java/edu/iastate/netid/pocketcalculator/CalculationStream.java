...
public class CalculationStream {
     ...
     * @throws NumberFormatException if either operand can't be interpreted as a double
     */
     ...
     * operand has a value, this returns zero. Useful for displaying the calculation stream in
     * a pocket calculator-like format.
     * @return the value of the current operand
     * @throws NumberFormatException if the current operand can't be interpreted as a double
     */
    public double getCurrentOperand() throws NumberFormatException {
        if (mCurrentOperation == Operation.NONE || mExpression2.toString().equals("")) {
            if(mExpression1.toString().equals("")) {
                return 0.0;
            }
